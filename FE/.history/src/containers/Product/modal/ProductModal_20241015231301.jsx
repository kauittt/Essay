...
const ProductModal = ({ toggle, data, action }) => {
    ...
    const validate = (values, t) => {
        ...
        requiredFields.forEach((field) => {
            console.log(field); // Debug để xem trường đang được xét
            console.log(values[field]); // Debug để xem giá trị của trường đó
            if (!values[field]) {
                // Kiểm tra nếu trường đó không có giá trị
                errors[field] = t("errors:validation.required"); // Gán lỗi cho trường đó
            }
        });
    ...
    categories = categories.filter((category) => category.name != "General");
