...
public class VoucherServiceTest {
    ...
    private final long VOUCHER_ID = 1;
    @BeforeEach
    public void setup() {
        entityManager.clear();
...
//    }
    ...
    public void testGetVoucherById_whenVoucherDoesNotExist_thenThrowException() {
       long nonExistentId = 1L;
        Mockito.when(voucherRepository.findById(eq(nonExistentId))).thenReturn(Optional.empty());

        Exception exception = assertThrows(RuntimeException.class, () -> {
            voucherService.getVoucherById(nonExistentId+"");
        });

        assertEquals(Response.notFound("Voucher", nonExistentId+""), exception.getMessage());
        Mockito.verify(voucherRepository, Mockito.times(1)).findById(eq(nonExistentId));
    }
    ...
    public void testUpdateVoucher_whenVoucherDoesNotExist_thenThrowException() {
       long nonExistentId = 1L;
        ...
        Exception exception = assertThrows(RuntimeException.class, () -> {
            voucherService.updateVoucher(nonExistentId+"", request);
        });
