...
public class UserServiceTest {
    ...
    public void setup() {
        ...
        user.setAuthorities(List.of(new Authority(1L, "ROLE_USER", new ArrayList<>())));
        ...
        response.setAuthorities(List.of(new AuthorityDTO(1L, "ROLE_USER")));
...
//
//        // THEN
//        assertEquals(responseList.size(), result.size());  // Ensure the size is expected
//        assertEquals("admin", result.get(0).getUsername()); // Check that the username matches
//
//        Mockito.verify(userRepository, Mockito.times(1)).findAll();  // Verify that findAll was called
//        Mockito.verify(userMapper, Mockito.times(1)).toDTO(any(User.class));  // Verify that toDTO was called
//    }
...
//    }
    ...
    public void testGetAllUsers_whenNoUsersExist_returnEmptyList() {
        ...
        Mockito.when(userRepository.findAll(Mockito.any(Sort.class))).thenReturn(emptyUserList);
        ...
        // Verify the findAll method was called with any Sort argument
        Mockito.verify(userRepository, Mockito.times(1)).findAll(Mockito.any(Sort.class));  // Match any Sort argument
    }
